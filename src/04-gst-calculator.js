...
export function calculateGST(amount, category) {
  if(amount <= 0 || !Number.isFinite(amount) || typeof category !== "string") return null;
  
  category = category.toLowerCase();

  const categoryGroup = ["essential", "food", "standard", "electronics", "luxury"];
  
  if(!categoryGroup.includes(category)) return null;
